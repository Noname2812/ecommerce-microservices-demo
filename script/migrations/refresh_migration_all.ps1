...
foreach ($serviceDirectory in $serviceDirectories) {
    ...
    $persistenceProject = Get-ChildItem `
        -Path $serviceDirectory.FullName `
        -Filter "UrbanX.*.Persistence.csproj" `
        -File `
        -Recurse |
    ...
    $persistenceDirectory =
        Split-Path -Parent $persistenceProject.FullName

    $migrationsDirectory =
        Join-Path $persistenceDirectory "Migrations"
    ...
    Push-Location $persistenceDirectory
