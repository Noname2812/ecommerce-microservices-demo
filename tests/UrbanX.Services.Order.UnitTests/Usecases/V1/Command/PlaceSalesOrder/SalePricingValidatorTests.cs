...
public class SalePricingValidatorTests
{
    ...
    public async Task ValidateAsync_PartialPriceDict_ReturnsSalePricingUnavailable()
    {
        var v1 = Guid.NewGuid();
        var v2 = Guid.NewGuid();
        var mock = new Mock<IPromotionServiceClient>();
        mock.Setup(x => x.GetSalePricesAsync(
                It.IsAny<Guid>(),
                It.IsAny<IReadOnlyList<PromotionSalePriceLine>>(),
                It.IsAny<CancellationToken>()))
            .ReturnsAsync(Result.Success(new Dictionary<Guid, decimal> { { v1, 10m } }));

        var validator = new SalePricingValidator(mock.Object);
    ...
    public async Task ValidateAsync_FullDictMatchingPrices_ReturnsSuccess()
    {
        ...
        mock.Setup(x => x.GetSalePricesAsync(
                ...
                It.IsAny<IReadOnlyList<PromotionSalePriceLine>>(),
                It.IsAny<CancellationToken>()))
            .ReturnsAsync(Result.Success(new Dictionary<Guid, decimal> { { v, 100m } }));
