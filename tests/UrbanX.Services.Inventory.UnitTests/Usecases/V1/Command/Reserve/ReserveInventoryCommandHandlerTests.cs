...
public class ReserveInventoryCommandHandlerTests
{
    ...
    /// <summary>Mirrors DB computed column so handler can read <see cref="InventoryItem.QuantityAvailable"/>.</summary>
    private static void SyncQuantityAvailable(InventoryItem item)
    {
        var v = item.QuantityOnHand - item.QuantityReserved;
        typeof(InventoryItem).GetProperty(nameof(InventoryItem.QuantityAvailable))!
            .SetValue(item, v);
    }

    [Fact]
    public async Task TC_INV_001_Handle_WhenStockSufficient_UpdatesReservedAndCreatesReservation()
    {
        var productId = Guid.Parse("10000000-0000-4000-8000-000000000001");
        var itemId = Guid.Parse("20000000-0000-4000-8000-000000000001");
        var line = new InventoryItem
        {
            Id = itemId,
            ProductId = productId,
            ProductName = "P",
        ...
        _items
            .Setup(r => r.GetTrackedPrimaryLinePerProductAsync(
                It.Is<IReadOnlyCollection<Guid>>(ids => ids.Single() == productId),
                It.IsAny<CancellationToken>()))
            .ReturnsAsync(new Dictionary<Guid, InventoryItem> { [productId] = line });
        ...
        Assert.Equal(3, line.QuantityReserved);
    ...
    [Fact]
    public async Task TC_INV_002_Handle_WhenInsufficientStock_ReturnsOutOfStock()
    {
    ...
    public async Task TC_INV_003_Handle_WhenPendingExists_ReturnsSameReservationId()
    {
        ...
        _items.Verify(
            r => r.GetTrackedPrimaryLinePerProductAsync(
                It.IsAny<IReadOnlyCollection<Guid>>(),
                It.IsAny<CancellationToken>()),
            Times.Never);
    }

    [Fact(Skip = "P2-T2 TC-INV-004: needs PostgreSQL + xmin concurrent load test (10 parallel vs 5 units on hand).")]
    ...
    public async Task TC_INV_005_Handle_WhenProductMissing_Returns422Error()
    {
        ...
        _items
            .Setup(r => r.GetTrackedPrimaryLinePerProductAsync(
                It.IsAny<IReadOnlyCollection<Guid>>(),
                It.IsAny<CancellationToken>()))
            .ReturnsAsync(new Dictionary<Guid, InventoryItem>());
