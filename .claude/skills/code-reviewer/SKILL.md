...
**Code hygiene**: Xóa toàn bộ `using` không dùng. Không để `using` thừa commit lên repo — gây noise diff và che khuất dependencies thực sự. Dùng IDE auto-clean (Roslyn analyzer `IDE0005`) hoặc `dotnet format`.
...
## Place Order Cross-Reference

Khi file thuộc flow đặt hàng (`*Order*`, `*Inventory*`, `*Coupon*`, `*Outbox*`, `*Compensation*`,
`*Reservation*`, `*Consumer*`), bắt buộc đối chiếu với **Place Order — Specific Rules**
và gắn tag `[PO-X]` vào từng issue tìm được.

| File pattern | Rules cần check |
|---|---|
| `PlaceOrderCommand`, `PlaceOrderHandler` | PO-1, PO-2, PO-3, PO-4, PO-9, PO-11, PO-12 |
| `InventoryReservation*`, `*ReserveInventory*` | PO-5, PO-7, PO-8, PO-10 |
| `CouponClaim*`, `*ClaimCoupon*` | PO-6, PO-7, PO-8, PO-10 |
| `OutboxMessage`, `OutboxWorker` | PO-3 |
| `CompensationOutbox*` | PO-4 |
| `*Consumer*`, `*EventHandler*` | PO-8 |

Nếu không rõ file có thuộc flow này không, ưu tiên check — false positive tốt hơn bỏ sót.
...
**Code quality**:
- ⚠️ Unused `using` directives — chạy `dotnet format` hoặc IDE0005
- ⚠️ File liên quan Place Order nhưng thiếu tag `[PO-X]` — đối chiếu Place Order rules